...
def train_model(start_epochs,  n_epochs, val_loss_min_input, 
    ...
    for epoch in range(start_epochs, n_epochs+1):
        ...
        for batch_idx, (sentences, targets) in enumerate(train_loader):
            ...
            # print('loss data in training', loss.item(), train_loss)
        ...
        with torch.no_grad():
            for batch_idx, (sentences, targets) in enumerate(val_loader):
                encoded_input = tokenizer(list(sentences), padding=True, truncation=True, max_length=64, return_tensors='pt')
                encoded_input = encoded_input.to(device)
                outputs = model(encoded_input)
                ...
                f1_samples_score = f1_score(val_target, np.where(val_output >= 0.5, 1, 0).astype(np.int8), average='samples')
