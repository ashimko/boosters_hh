...
from helper import _process_pred_labels, load_treshold, save_predictions, load_catboost_model
from model_config import MODEL_NAME, N_SPLITS
from model import get_model


def predict():
    test = pd.concat([pd.read_pickle(os.path.join(SBER_LARGE_NLU_RU_PATH, path)) 
                      for path in os.listdir(SBER_LARGE_NLU_RU_PATH) if path.startswith('test')], axis=1)
